...
- **Quantization**: Group continuous FICO scores into categories or "buckets" using optimisation techniques.
- **Optimise bucket boundaries**: The task explores different approaches to finding the best bucket boundaries, such as minimising mean squared error (MSE) or maximising log-likelihood.
...
2. **Bucket Optimisation**: Use optimisation techniques such as log-likelihood maximisation to determine the boundaries that best represent the risk associated with different FICO score ranges.
3. **Categorisation**: Assign each FICO score into a bucket based on the optimised boundaries.
...
- `optimize(fico_scores, defaults, num_buckets)`: A function that finds the optimal bucket boundaries by maximising the log-likelihood function through random sampling and iteration.
